# Archaludon ex / 铝钢桥龙 ex

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_315_archaludon_ex.csv`  
Kaggle 统计 archetype: `Archaludon`
...
|---|---|---|---|---|
|f2a48b323dd0|980.8|2|0.000|pepe:1;sadwat:1|

## 分段分布

|band|rows|WR|max_score|top sigs|
|---|---|---|---|---|
|900-999|2|0.000|980.8|f2a48b323dd0:2|

## 打法摘要

Metal energy tempo。Kaggle 0812/0813 样本很少，但用户提交胡地时曾被该类卡组击败，因此 RR 池不能完全忽略。
...
缺少高分样本，优先从 0801-0815 全部 900+ 或特定 high team 数据补充。

## 单独训练

目标是先训练一个 deck-sig specialist，而不是把多个不同 game plan 混在一起。当前自动填入的 `DECK_SIG=f2a48b323dd0` 来自 2026-08-13 ladder 强签名表。

```bash
export CORPUS=${CORPUS:-data/bc_corpus_banded_latest}
export ARCHETYPE='Archaludon'
export DECK_SIG=f2a48b323dd0
export DECK='decks/pool_315_archaludon_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_archaludon_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_archaludon_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_archaludon_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_archaludon_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_archaludon_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/archaludon
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/archaludon/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/archaludon/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/archaludon/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/archaludon
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/archaludon/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/archaludon/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate archaludon="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/archaludon/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_archaludon
mkdir -p "$REPLAY_DIR"
...
- [YouTube search: Archaludon ex gameplay](https://www.youtube.com/results?search_query=Archaludon+ex+Pokemon+TCG+gameplay+deck+profile)
- [Bilibili search: 铝钢桥龙 ex](https://search.bilibili.com/all?keyword=%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%8D%A1%E7%89%8C+%E9%93%9D%E9%92%A2%E6%A1%A5%E9%BE%99+ex+%E5%8D%A1%E7%BB%84)
