...
export ARCHETYPE='Sylveon Safeguard / 仙子伊布 Safeguard'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_373_sylveon_safeguard.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_sylveon_safeguard_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_sylveon_safeguard_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_sylveon_safeguard_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_sylveon_safeguard_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_sylveon_safeguard_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/sylveon_safeguard
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/sylveon_safeguard/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/sylveon_safeguard/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/sylveon_safeguard/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/sylveon_safeguard
...
python3 tools/eval_round_robin.py \
  --entry sylveon_safeguard="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/sylveon_safeguard/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/sylveon_safeguard/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate sylveon_safeguard="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/sylveon_safeguard/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_sylveon_safeguard
mkdir -p "$REPLAY_DIR"
