...
目标是先训练一个 deck-sig specialist，而不是把多个不同 game plan 混在一起。当前自动填入的 `DECK_SIG=aa80b030c069` 来自 2026-08-13 ladder 强签名表。

```bash
export CORPUS=${CORPUS:-data/bc_corpus_banded_latest}
export ARCHETYPE='N'"'"'s Zoroark'
export DECK_SIG=aa80b030c069
export DECK='decks/pool_320_n_s_zoroark_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_n_zoroark_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_n_zoroark_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_n_zoroark_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_n_zoroark_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_n_zoroark_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/n_zoroark
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/n_zoroark/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/n_zoroark/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/n_zoroark/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/n_zoroark
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/n_zoroark/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/n_zoroark/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate n_zoroark="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/n_zoroark/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_n_zoroark
mkdir -p "$REPLAY_DIR"
