...
目标是先训练一个 deck-sig specialist，而不是把多个不同 game plan 混在一起。当前自动填入的 `DECK_SIG=cc2e995b5ad0` 来自 2026-08-13 ladder 强签名表。

```bash
export CORPUS=${CORPUS:-data/bc_corpus_banded_latest}
export ARCHETYPE='Dragapult'
export DECK_SIG=cc2e995b5ad0
export DECK='decks/pool_284_dragapult_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_dragapult_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_dragapult_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_dragapult_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_dragapult_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_dragapult_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/dragapult
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/dragapult/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/dragapult/random_g500.log
```
...
该 deck 当前优先关注的低胜率/接近五五对手: Crustle Wall、Mega Lopunny、Cynthia Garchomp、Marnie Grimmsnarl。

先构建一个每类 top2 的轻量 RR 池:

```bash
export RR_MANIFEST=${RR_MANIFEST:-logs/rr_pool_latest/filtered_balanced.csv}
export FOCUS_MANIFEST=logs/deck_eval/dragapult/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/dragapult
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/dragapult/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/dragapult/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate dragapult="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/dragapult/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_dragapult
mkdir -p "$REPLAY_DIR"
