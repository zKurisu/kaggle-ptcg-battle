# Raging Bolt ex / 猛雷鼓 ex

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_280_raging_bolt_ex.csv`  
...
Basic ex burst/race。真实 PTCG 中常与 Ogerpon engine 绑定，通过大量能量和高打点快速拿奖。

## 关键牌

- Raging Bolt ex
- Teal Mask Ogerpon ex
- Sandy Shocks/energy engine
...
本地 ladder 分类没有单独稳定统计；若出现在 episode，应单独建 archetype classifier。
...
export ARCHETYPE='Raging Bolt ex / 猛雷鼓 ex'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_280_raging_bolt_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_raging_bolt_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_raging_bolt_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_raging_bolt_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_raging_bolt_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_raging_bolt_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/raging_bolt
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/raging_bolt/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/raging_bolt/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/raging_bolt/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/raging_bolt
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/raging_bolt/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/raging_bolt/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate raging_bolt="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/raging_bolt/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_raging_bolt
mkdir -p "$REPLAY_DIR"
...
- [YouTube search: Raging Bolt Ogerpon gameplay](https://www.youtube.com/results?search_query=Raging+Bolt+Ogerpon+Pokemon+TCG+gameplay+deck+profile)
- [Bilibili search: 猛雷鼓 厄诡椪](https://search.bilibili.com/all?keyword=%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%8D%A1%E7%89%8C+%E7%8C%9B%E9%9B%B7%E9%BC%93+%E5%8E%84%E8%AF%A1%E6%A4%AA+%E5%8D%A1%E7%BB%84)
