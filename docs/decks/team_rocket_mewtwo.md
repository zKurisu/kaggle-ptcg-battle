# Team Rocket's Mewtwo ex / 火箭队的超梦 ex

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_337_rocket_s_mewtwo_ex.csv`  
Kaggle 统计 archetype: `Team Rocket Mewtwo`
...
|---|---|---|---|---|
|4c6c10c0d8d5|1159.2|6|0.667|Majkel1337:6|
|2878a7405032|962.1|13|0.615|kashiwashira:13|

## 分段分布

|band|rows|WR|max_score|top sigs|
|---|---|---|---|---|
|1100-1199|6|0.667|1159.2|4c6c10c0d8d5:6|
|900-999|13|0.615|962.1|2878a7405032:13|

## 打法摘要

Team Rocket board-count payoff。Mewtwo 往往需要足够火箭队宝可梦在场后才能进入主攻击节奏，因此 setup quality 比单回合打点更重要。
...
|---|---|---|---|
|高胜率|Alakazam|372|0.777|
|高胜率|Crustle Wall|171|0.509|
|高胜率|Marnie Grimmsnarl|1602|0.476|
|高胜率|Cynthia Garchomp|122|0.434|
|高胜率|Teal Mask Ogerpon|143|0.350|
|低胜率/接近五五|无足够非重复样本|-|-|

## 训练和评测注意事项

random 可以很高但 Kaggle 转化差，说明模型可能学到基础启动但缺少 matchup plan。

## 单独训练

目标是先训练一个 deck-sig specialist，而不是把多个不同 game plan 混在一起。当前自动填入的 `DECK_SIG=4c6c10c0d8d5` 来自 2026-08-13 ladder 强签名表。

```bash
export CORPUS=${CORPUS:-data/bc_corpus_banded_latest}
export ARCHETYPE='Team Rocket Mewtwo'
export DECK_SIG=4c6c10c0d8d5
export DECK='decks/pool_337_rocket_s_mewtwo_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_team_rocket_mewtwo_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_team_rocket_mewtwo_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_team_rocket_mewtwo_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_team_rocket_mewtwo_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_team_rocket_mewtwo_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/team_rocket_mewtwo
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/team_rocket_mewtwo/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/team_rocket_mewtwo/random_g500.log
```
...
该 deck 当前优先关注的低胜率/接近五五对手: Teal Mask Ogerpon、Cynthia Garchomp、Marnie Grimmsnarl、Crustle Wall。

先构建一个每类 top2 的轻量 RR 池:

```bash
export RR_MANIFEST=${RR_MANIFEST:-logs/rr_pool_latest/filtered_balanced.csv}
export FOCUS_MANIFEST=logs/deck_eval/team_rocket_mewtwo/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/team_rocket_mewtwo
...
python3 tools/eval_round_robin.py \
  --entry team_rocket_mewtwo="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/team_rocket_mewtwo/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/team_rocket_mewtwo/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate team_rocket_mewtwo="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/team_rocket_mewtwo/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_team_rocket_mewtwo
mkdir -p "$REPLAY_DIR"
...
python3 tools/make_kaggle_opp_round_robin_cmd.py \
  --policy-name team_rocket_mewtwo \
  --policy "$POLICY" \
  --deck "$DECK" \
...
- [YouTube search: Team Rocket's Mewtwo ex gameplay](https://www.youtube.com/results?search_query=Team+Rocket%27s+Mewtwo+ex+Pokemon+TCG+gameplay+deck+profile)
- [Bilibili search: 火箭队的超梦](https://search.bilibili.com/all?keyword=%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%8D%A1%E7%89%8C+%E7%81%AB%E7%AE%AD%E9%98%9F%E7%9A%84%E8%B6%85%E6%A2%A6+ex+%E5%8D%A1%E7%BB%84)
