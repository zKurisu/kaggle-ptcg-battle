...
def _value_fn(numpy_policy, observation: dict) -> float:
    ...
    try:
        ...
        history = (
            numpy_policy._history_arrays(observation)  # noqa: SLF001 - MCTS leaf eval should match live policy.
            if hasattr(numpy_policy, "_history_arrays")
            else None
        )
        h = numpy_policy.encode_state(
            d.board_cards,
            d.hand_cards,
            d.state_feats,
            history,
