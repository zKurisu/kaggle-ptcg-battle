...
export ARCHETYPE='Mega Greninja ex / 超级甲贺忍蛙 ex'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_370_mega_greninja_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_mega_greninja_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_mega_greninja_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_mega_greninja_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_mega_greninja_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_mega_greninja_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/mega_greninja
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/mega_greninja/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/mega_greninja/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/mega_greninja/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/mega_greninja
...
python3 tools/eval_round_robin.py \
  --entry mega_greninja="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/mega_greninja/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/mega_greninja/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate mega_greninja="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/mega_greninja/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_mega_greninja
mkdir -p "$REPLAY_DIR"
