# Hop's Trevenant / 赫普朽木妖

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_363_hop_s_trevenant.csv`  
Kaggle 统计 archetype: `Hop Trevenant`
...
Stage-1/2 control variant。0812 高分 band 只出现极少行，不能直接当主流。
...
export ARCHETYPE='Hop Trevenant'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_363_hop_s_trevenant.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_hop_trevenant_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_hop_trevenant_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_hop_trevenant_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_hop_trevenant_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_hop_trevenant_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/hop_trevenant
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/hop_trevenant/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/hop_trevenant/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/hop_trevenant/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/hop_trevenant
...
python3 tools/eval_round_robin.py \
  --entry hop_trevenant="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/hop_trevenant/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/hop_trevenant/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate hop_trevenant="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/hop_trevenant/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_hop_trevenant
mkdir -p "$REPLAY_DIR"
...
- [YouTube search: Hop's Trevenant gameplay](https://www.youtube.com/results?search_query=Hop%27s+Trevenant+Pokemon+TCG+gameplay+deck+profile)
- [Bilibili search: 赫普 朽木妖](https://search.bilibili.com/all?keyword=%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%8D%A1%E7%89%8C+%E8%B5%AB%E6%99%AE+%E6%9C%BD%E6%9C%A8%E5%A6%96+%E5%8D%A1%E7%BB%84)
