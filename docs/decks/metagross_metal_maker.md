# Metagross Metal Maker / 巨金怪 Metal Maker

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_361_metagross_metal_maker.csv`  
...
Metal acceleration Stage-2。核心是通过 Metal Maker/金属能加速建立连续攻击。
...
Azul 有 Pitch Black decklist，可作为 strategy seed；Kaggle evidence 需要另行确认。
...
export ARCHETYPE='Metagross Metal Maker / 巨金怪 Metal Maker'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_361_metagross_metal_maker.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_metagross_metal_maker_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_metagross_metal_maker_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_metagross_metal_maker_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_metagross_metal_maker_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_metagross_metal_maker_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/metagross_metal_maker
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/metagross_metal_maker/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/metagross_metal_maker/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/metagross_metal_maker/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/metagross_metal_maker
...
python3 tools/eval_round_robin.py \
  --entry metagross_metal_maker="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/metagross_metal_maker/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/metagross_metal_maker/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate metagross_metal_maker="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/metagross_metal_maker/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_metagross_metal_maker
mkdir -p "$REPLAY_DIR"
...
python3 tools/make_kaggle_opp_round_robin_cmd.py \
  --policy-name metagross_metal_maker \
  --policy "$POLICY" \
  --deck "$DECK" \
...
- [YouTube search: Metagross Metal Maker gameplay](https://www.youtube.com/results?search_query=Metagross+Metal+Maker+Pokemon+TCG+gameplay+deck+profile)
