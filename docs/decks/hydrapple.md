# Hydrapple ex / 蜜集大蛇 ex

资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_352_hydrapple_ex.csv`  
...
Grass evolution/resource deck。现实 Limitless 中有 meta 占比，Kaggle 分类暂未稳定单列。

## 关键牌

- Applin
- Dipplin/Hydrapple ex
- grass energy engine
- healing/acceleration support
...
后续可以用 Limitless strong list 先做 deck conversion，再训练 shadow。
...
export ARCHETYPE='Hydrapple ex / 蜜集大蛇 ex'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_352_hydrapple_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_hydrapple_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_hydrapple_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_hydrapple_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_hydrapple_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_hydrapple_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/hydrapple
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/hydrapple/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/hydrapple/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/hydrapple/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/hydrapple
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/hydrapple/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/hydrapple/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate hydrapple="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/hydrapple/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_hydrapple
mkdir -p "$REPLAY_DIR"
...
- [YouTube search: Hydrapple ex gameplay](https://www.youtube.com/results?search_query=Hydrapple+ex+Pokemon+TCG+gameplay+deck+profile)
- [Bilibili search: 蜜集大蛇 ex](https://search.bilibili.com/all?keyword=%E5%AE%9D%E5%8F%AF%E6%A2%A6%E5%8D%A1%E7%89%8C+%E8%9C%9C%E9%9B%86%E5%A4%A7%E8%9B%87+ex+%E5%8D%A1%E7%BB%84)
