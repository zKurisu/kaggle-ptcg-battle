...
资料更新时间: 2026-08-17  
本地 deck 模板: `decks/pool_371_beedrill_ex.csv`  
...
Grass evolution tempo/disruption。具体要看 decklist 是否走纯进化攻击或异常状态/干扰路线。
...
- Kakuna
- Beedrill ex
- grass/search package
- poison/disruption tools
...
Azul 有 Pitch Black decklist，可作为公开策略入口；Kaggle 样本需补。
...
export ARCHETYPE='Beedrill ex / 大针蜂 ex'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_371_beedrill_ex.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_beedrill_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_beedrill_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_beedrill_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_beedrill_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_beedrill_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/beedrill
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/beedrill/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/beedrill/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/beedrill/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/beedrill
...
python3 tools/eval_round_robin.py \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/beedrill/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/beedrill/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate beedrill="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/beedrill/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_beedrill
mkdir -p "$REPLAY_DIR"
...
- [YouTube search: Beedrill ex gameplay](https://www.youtube.com/results?search_query=Beedrill+ex+Pokemon+TCG+gameplay+deck+profile)
