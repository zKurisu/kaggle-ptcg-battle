...
export ARCHETYPE='Slowking Seek Inspiration / 呆壳兽灵感检索'
export DECK_SIG=REPLACE_WITH_DECK_SIG
export DECK='decks/pool_322_slowking_seek_inspiration.csv'
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0} python3 -u tools/bc2_train.py \
  ...
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_slowking_seek_inspiration_${DECK_SIG}_single.npz" \
  2>&1 | tee "$LOG_DIR/bc2_slowking_seek_inspiration_${DECK_SIG}_single.log"
```
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-1} python3 -u tools/bc2_train.py \
  ...
  --load-progress-every 200000 \
  --checkpoint-every 1 \
  --save "$OUT_DIR/bc2_slowking_seek_inspiration_${DECK_SIG}_cross_stepplan.npz" \
  2>&1 | tee "$LOG_DIR/bc2_slowking_seek_inspiration_${DECK_SIG}_cross_stepplan.log"
```
...
```bash
export POLICY="$OUT_DIR/bc2_slowking_seek_inspiration_${DECK_SIG}_single.npz"
mkdir -p logs/deck_eval/slowking_seek_inspiration
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/slowking_seek_inspiration/random_g300.log
```
...
python3 tools/eval_bc.py "$POLICY" \
  ...
  --progress-every 50 \
  --max-turns 700 \
  2>&1 | tee logs/deck_eval/slowking_seek_inspiration/random_g500.log
```
...
export FOCUS_MANIFEST=logs/deck_eval/slowking_seek_inspiration/rr_pool_top2_per_arch.csv
mkdir -p logs/deck_eval/slowking_seek_inspiration
...
python3 tools/eval_round_robin.py \
  --entry slowking_seek_inspiration="$POLICY:$DECK" \
  ...
  --max-turns 700 \
  --progress-every 20 \
  --out-csv logs/deck_eval/slowking_seek_inspiration/rr_top2_per_arch_g100.csv \
  2>&1 | tee logs/deck_eval/slowking_seek_inspiration/rr_top2_per_arch_g100.log
```
...
python3 tools/trace_matchup_decisions.py \
  --candidate slowking_seek_inspiration="$POLICY:$DECK" \
  --opponent "$OPP_ENTRY" \
  --games 20 \
  --seed 20260817 \
  --max-turns 700 \
  --progress-every 1 \
  --out-prefix logs/deck_eval/slowking_seek_inspiration/trace_vs_bad_opp
```
...
export REPLAY_DIR=logs/kaggle_replay_${SUB_ID}_slowking_seek_inspiration
mkdir -p "$REPLAY_DIR"
...
python3 tools/make_kaggle_opp_round_robin_cmd.py \
  --policy-name slowking_seek_inspiration \
  --policy "$POLICY" \
  --deck "$DECK" \
